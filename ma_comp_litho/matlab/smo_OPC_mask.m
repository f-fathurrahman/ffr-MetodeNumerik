function [] = smo_OPC_mask(N, pz, N_filter, pixel, k, NA, lamda, order, sigma_large_inner, sigma_large_outer);
...
% sigma_large_inner=0.4;   %Inner Partial coherence factor
% sigma_large_outer=0.5;   %Outer Partial coherence factor
...
error=1000000;   %Output pattern error in the current iteration
...
while (error > epsilon )   
   ...
   for p=1:N_coherence
       for q=1:N_coherence   
           if (yita(p,q)>0)
               ...
               direction_mask=direction_mask+(2)*yita(p,q)/normalize* real(  imfilter( (pz-aerial).*imfilter(double(m),h.*exponential) , conj(g.*exponential) )  +   imfilter( (pz-aerial).*imfilter(double(m),conj(h.*exponential)) , g.*exponential )    );
           end
       end
   end
   ...
   %%%%%%If no pixel on mask changed, then the program is terminated%%%%%%
   ...
   while (sum(sum(flag_mask))+sum(sum(flag_source))>0)   %Check whether there is any changable pixel on mask
       ...
       if (test_mask(max_row_mask,max_column_mask) >= 0)
           ...
           if ((m(max_row_mask,max_column_mask)==0)&(direction_mask(max_row_mask,max_column_mask)>0)) | ((m(max_row_mask,max_column_mask)==1)&(direction_mask(max_row_mask,max_column_mask)<0))
               ...
               if(error>=error_pre)   %If the error is increased, then restore the original pixel values
                   ...
                   error_pre=error;
                   disp('Change mask');
                   disp(strcat('Output pattern error=',num2str(error)));
               end
           end
       end
   end
...
sum10=sum(sum((pz-aerial).^2));   %The output pattern error of initial mask and source
...
sum6=sum(sum((pz-aerial).^2));   %The output pattern error of optimized mask and source
