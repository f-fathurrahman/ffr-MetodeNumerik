function [] = smo_PSM(N, pz, N_filter, pixel, k, NA, lamda, order, sigma);
...
while (error > threshold )   
   ...
   for p=1:N_coherence
       for q=1:N_coherence   
           ...
           direction_source(p,q)=-1*sum(sum( (abs(pz)-aerial).* abs(imfilter(double(m),h.*exponential)).^2 )) *(-2)/normalize; 
       end
   end
