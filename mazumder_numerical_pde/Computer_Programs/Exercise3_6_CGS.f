...
! using the finite-difference method and the CGS method.
! The right BC is of Neumann type, while the other 3 are of Dirichlet type.
! Exercise 3.6
...
! Declaration of variables
    ...
    REAL(8), DIMENSION(n,m) :: S,phi
    REAL(8) :: dx,dx2,dy,dy2
    REAL(8) :: st_fac_x,st_fac_y,dx0,dy0
    REAL(8) :: ae,aw,as,an,ao,source,res,fde,delta,dtq,alpha,beta,delta0,delta1
    REAL(4) :: ta(2),time_start,time_end
    

! Open output file
    OPEN(unit=10, file="Exercise3_6_CGS.out", status = "unknown")
    OPEN(unit=20, file="Exercise3_6_CGS.rsl", status = "unknown")
    ...
    DO i = 1,n
      x(i) = (i-1)*dx
      phi_bot(i) = zero !bottom
      phi_top(i) = zero !top
    ENDDO
    DO j = 1,m
      y(j) = (j-1)*dy
      phi_left(j) = zero !left
    ENDDO
    DO i = 1,n
      DO j = 1,m
        S(i,j) = one
...
! Right Boundary Nodes
    ...
    DO j = 2,m-1
      k = (j-1)*n + i
      RR(k) = QQQ(k) - EEE(k)*SSS(k) - HHH(k)*SSS(k+n) - &
                DDD(k)*SSS(k-1) - BBB(k)*SSS(k-n)
    ENDDO
    RR0(:) = RR(:)
    ds(:) = zero
