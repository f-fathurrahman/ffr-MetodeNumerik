...
! using the finite-difference method and the Method of Steepest Descent
! Exercise 3.4(e)
...
! Declaration of variables
    ...
    REAL(8) :: ae,aw,as,an,ao,source,res,fde,delta,rtq,alpha
    REAL(4) :: ta(2),time_start,time_end
    

! Open output file
    OPEN(unit=10, file="Exercise3_4_msd.out", status = "unknown")
    OPEN(unit=20, file="Exercise3_4_msd.rsl", status = "unknown")
...
! Compute vector q
      ...
      DO i = 2, n-1
        DO j = 2,m-1
          k = (j-1)*n + i
          q(k) =  EEE(k)*RR(k) + FFF(k)*RR(k+1) + HHH(k)*RR(k+n) + &
                  DDD(k)*RR(k-1) + BBB(k)*RR(k-n)
        ENDDO
      ENDDO
