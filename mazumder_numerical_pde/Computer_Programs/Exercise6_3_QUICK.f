...
! Open output file
    OPEN(unit=10, file="Exercise6_3_QUICK.out", status = "unknown")
...
! Set up coefficient matrix and source vector
    a(:,:)=zero; b(:) = zero
    a(1,1) = 12.0d0+3.0d0*Pe*dx
    a(1,2) = (Pe*dx-4.0d0)
    b(1) = (8.0d0+4.0d0*Pe*dx)*phi_left
    a(2,1) = -(24.0d0+27.0d0*Pe*dx)
    a(2,2) = (48.0d0+10.0d0*Pe*dx)
    a(2,3) = 9.0d0*Pe*dx-24.0d0
    b(2) = -8.0d0*Pe*dx*phi_left
    DO i = 3,nc-1
      a(i,i-1) = -(8.0d0+7.0d0*Pe*dx)
      a(i,i) = 16.0d0+3.0d0*Pe*dx
      a(i,i+1) = 3.0d0*Pe*dx-8.0d0
      a(i,i-2) = Pe*dx
      b(i) = zero
    ENDDO
    a(nc,nc) = 96.0d0-9.0d0*Pe*dx
    a(nc,nc-1) = -(32.0d0+18.0d0*Pe*dx)
    a(nc,nc-2) = 3.0d0*Pe*dx
    b(nc) = (64.0d0 - 24.0d0*Pe*dx)*phi_right
