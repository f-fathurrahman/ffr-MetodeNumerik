...
! using the finite-difference method and the Conjugate Gradient Squared
! method on a non-uniform mesh
! Exercise 3.5
...
! Open output file
    OPEN(unit=10, file="Exercise3_5.out", status = "unknown")
    OPEN(unit=20, file="Exercise3_5.rsl", status = "unknown")

    IF(st > one)THEN
      st_fac_x = (st - st**n)/(one-st)
      st_fac_y = (st - st**m)/(one-st)
    ELSE
      st_fac_x = n-1
      st_fac_y = m-1
    ENDIF
...
! Interior Nodes
    DO i = 2, n-1
      DO j = 2,m-1
        k = (j-1)*n + i
        EEE(k) = 2.0d0/(dx(i)*dx(i-1))+2.0d0/(dy(j)*dy(j-1))
        BBB(k) = -2.0d0/(dy(j-1)*(dy(j-1)+dy(j)))
        HHH(k) = -2.0d0/(dy(j)*(dy(j-1)+dy(j)))
        DDD(k) = -2.0d0/(dx(i-1)*(dx(i-1)+dx(i)))
        FFF(k) = -2.0d0/(dx(i)*(dx(i-1)+dx(i)))
