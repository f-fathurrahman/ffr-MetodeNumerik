...
program main
  ...
  IMPLICIT NONE
  INTEGER(4), PARAMETER :: n_nodes = 17
  INTEGER(4) :: i,approach
  ...
  ! Open output file
  OPEN(unit=10, file="TEMP_example2_2.dat", status = "unknown")
...
! Boundary conditions
  IF(approach == 1)THEN
    a(1,1) = -3.0d0/(2.0d0*dx)
    a(1,2) = 2.0d0/dx
    a(1,3) = -1.0d0/(2.0d0*dx)
    b(1) = deriv_left
  ELSE
    a(1,1) = -3.5d0/dx2
    a(1,2) = 4.0d0/dx2
    a(1,3) = -0.5d0/dx2
    b(1) = s(1) + deriv_left*3.0d0/dx
  ENDIF
