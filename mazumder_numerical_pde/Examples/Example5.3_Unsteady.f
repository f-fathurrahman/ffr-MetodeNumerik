...
! Unsteady (Time Marching) approach to solving a PDE
! Example 5.3.
...
! Declaration of variables
    ...
    INTEGER(4), PARAMETER :: max_iter = 1000, max_times = 1000
    INTEGER(4) :: i,j,iter,it
    ...
    REAL(8) :: tol = 1.0d-3, tol_time = 1.0D-4
    REAL(8), DIMENSION(n) :: x,phi_bot,phi_top
    REAL(8), DIMENSION(m) :: y,phi_left,phi_right
    REAL(8), DIMENSION(n,m) :: S,phi,phian,phio
    REAL(8) :: dx,dx2,dy,dy2,dt
    REAL(8) :: ae,aw,as,an,ao,source,res,fde,res1,sumt
    REAL(4) :: ta(2),time_start,time_end
    

! Open output file
    OPEN(unit=10, file="Example5_3_Unsteady2.out", status = "unknown")
    OPEN(unit=20, file="Example5_3_Unsteady2.rsl", status = "unknown")
...
! Start of time loop
    ...
    WRITE(20,*) "Residual at time step =", it
...
! Monitor Steady State
    ...
    DO i = 1,n
      DO j = 1,m
        sumt = sumt + (phi(i,j)-phio(i,j))**2
      ENDDO
    ENDDO
    sumt = SQRT(MAX(zero,sumt))
    IF(sumt < tol_time) EXIT
