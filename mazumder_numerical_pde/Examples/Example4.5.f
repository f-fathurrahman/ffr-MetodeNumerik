...
! the CD scheme and the Gauss-Seidel method with inertial damping.
! Example 4.5
...
! Declaration of variables
    IMPLICIT NONE
    INTEGER(4), PARAMETER :: n_nodes = 11, max_iter = 100
    INTEGER(4) :: i,iter
...
! Open output file
    OPEN(unit=10, file="Example4_5.rsl", status = "unknown")
...
! GS Update
      DO i = 2,n_nodes-1
        ...
        aw = -(half+p)
        ao = two*p*(one+alpha)
        phi(i) = (-ae*phi(i+1)-aw*phi(i-1)+alpha*two*p*phi(i))/ao
      ENDDO
