...
! Example 5.4: Crank-Nicolson Method
...
! Open output file
    OPEN(unit=10, file="Example5_4_CN.out", status = "unknown")
    OPEN(unit=20, file="Example5_4_CN_point.out", status = "unknown")
...
! Numerical Solution
      ...
      DO i = 2,n_nodes-1
        ...
        c(i) = -one/(two*dx2)
        a(i-1) = -one/(two*dx2)
        b(i) = phi(itimes,i)/dt + (phi(itimes,i+1)-two*phi(itimes,i)+phi(itimes,i-1))/(two*dx2)
      ENDDO
      d(n_nodes) = one
      b(n_nodes) = phi_r
