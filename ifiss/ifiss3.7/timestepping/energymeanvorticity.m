function [ke,acc,meanv]=energymeanvorticity(qmethod,ev,U,Udot,tt,By,Bx,G,xy,tout,fig,noffset)
%ENERGYMEANVORTICITY compute acceleration and mean vorticity
%     [ke,acc,meanv] = energymeanvorticity(qmethod,mv,U,Udot,xtime,By,Bx,G,xy,1,fig,noffset);
%   input
...
if tout, figure(fig),
subplot(121)
plot(1+noffset:nstep+noffset,meanv,'oc'), axis square
hold on, plot(1+noffset:nstep+noffset,meanv,'-b'), hold on
title('mean vorticity')
xlabel('timestep')
%xlabel('time')
subplot(122)
semilogy(1+noffset:nstep+noffset,acc,'-k'), axis square
hold on, semilogy(1+noffset:nstep+noffset,acc,'.-b'), hold on
