...
%
figure(2)
xp=[bndxy(1,1),bndxy(2,1)]; yp=[bndxy(1,2),bndxy(2,2)];    %  bottom of the channel
plot(xp,yp,'-k','Linewidth',3); hold on
xp=[bndxy(2,1),bndxy(3,1)]; yp=[bndxy(2,2),bndxy(3,2)];    %  outflow boundary
plot(xp,yp,'-g','Linewidth',3); hold on
xp=[bndxy(3,1),bndxy(4,1)]; yp=[bndxy(3,2);bndxy(4,2)];    %  top of the channel
plot(xp,yp,'-k','Linewidth',3);
xp=[bndxy(4,1),bndxy(1,1)]; yp=[bndxy(4,2),bndxy(1,2)];    %  inflow boundary
plot(xp,yp,'-r','Linewidth',3); hold on
xp=[bndxy(5,1),bndxy(6,1)]; yp=[bndxy(5,2),bndxy(6,2)];    %  bottom of the obstacle
plot(xp,yp,'-k','Linewidth',2); hold on 
xp=[bndxy(6,1),bndxy(7,1)]; yp=[bndxy(6,2),bndxy(7,2)];    %  right boundary of the obsracle
plot(xp,yp,'-k','Linewidth',2); hold on
xp=[bndxy(7,1),bndxy(8,1)]; yp=[bndxy(7,2),bndxy(8,2)];    %  top of the obstacle
plot(xp,yp,'-k','Linewidth',2); hold on
xp=[bndxy(8,1),bndxy(5,1)]; yp=[bndxy(8,2),bndxy(5,2)];    %  left boundary of the obstacle
plot(xp,yp,'-k','Linewidth',2); hold on
axis([bndxy(1,1)-0.2 bndxy(2,1)+0.2 bndxy(1,2)-0.2 bndxy(4,2)+0.2]);
axis equal; axis off;
obst_dom=gca;
save obst_ns_plot.mat obst_dom
close(2)
return
