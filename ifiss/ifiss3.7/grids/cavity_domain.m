...
%
figure(2)
xp=[bndxy(1,1),bndxy(2,1)]; yp=[bndxy(1,2),bndxy(2,2)];    %  bottom of the cavity
plot(xp,yp,'-k','Linewidth',3); hold on
xp=[bndxy(2,1),bndxy(3,1)]; yp=[bndxy(2,2),bndxy(3,2)];    %  right vertical boundary
plot(xp,yp,'-k','Linewidth',3); hold on
xp=[bndxy(3,1),bndxy(4,1)]; yp=[bndxy(3,2),bndxy(4,2)];    %  top of the cavity
...
axis equal; axis off;
cavity_dom=gca;
save cavity_ns_plot.mat cavity_dom
close(2)
return
