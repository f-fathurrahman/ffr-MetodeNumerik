...
%
%  Plot the domain shape for the solution
%
figure(2)
plot([-1 -1],[0 1],'-k','Linewidth',2); hold on    %  inflow
plot([-1 L],[1 1],'-b','Linewidth',2); hold on     %  top of the channel 
plot([L L],[1 -1],'-k','Linewidth',2); hold on     %  outflow
plot([L 0],[-1 -1],'-r','Linewidth',2); hold on    %  bottom wide channel
plot([0 0],[-1 0],'-r','Linewidth',2); hold on     %  bottom transition
plot([0 -1],[0 0],'-r','Linewidth',2); hold on     %  bottom narrow channel 
axis([-1.2 L+0.2 -1.2 1.2]);
axis off
step_dom=gca;
gohome; cd datafiles
save step_bouss_plot.mat step_dom
close(2)
%
