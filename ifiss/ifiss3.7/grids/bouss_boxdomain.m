...
% Boussinesq case
save box_grid1h.mat grid str H L gh nh hbbc hty net
fprintf('Grid data saved in box_grid1h.mat.\n')
%
%% Plot the domain geometry for movies
%
figure(2)
xp=[0,L]; yp=[0,0];   %  bottom straight segment
switch(hty)   %  cavity type
...
switch(hty)   %  cavity type
   case(1)    %  Rayleigh-Benard
      plot(xp,yp,'-k','Linewidth',3); hold on
   case(2)    %  laterally heated
      plot(xp,yp,'-b','Linewidth',3); hold on
end
xp=[L,0]; yp=[H,H];
switch(hty)   %  cavity type
   case(1)    %  Rayleigh-Benard
...
axis([-0.2 L+0.2 -0.2 H+0.2]);
axis equal
box_dom=gca;
save box_bouss_plot.mat box_dom
close(2);
return
