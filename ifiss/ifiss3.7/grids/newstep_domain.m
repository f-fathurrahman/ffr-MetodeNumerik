...
xp=[bndxy(4,1),bndxy(5,1)]; yp=[bndxy(4,2),bndxy(5,2)];    %  top of the channel
plot(xp,yp,'-k','Linewidth',3); hold on
xp=[bndxy(5,1),bndxy(6,1)]; yp=[bndxy(5,2),bndxy(6,2)];    %  inflow boundary
plot(xp,yp,'-r','Linewidth',3); hold on
xp=[bndxy(6,1),bndxy(7,1)]; yp=[bndxy(6,2),bndxy(7,2)];    %  bottom of the narrow channel
plot(xp,yp,'-k','Linewidth',3); hold on
xp=[bndxy(7,1),bndxy(2,1)]; yp=[bndxy(7,2),bndxy(2,2)];    %  the expansion
plot(xp,yp,'-k','Linewidth',3); hold on
axis([bndxy(1,1)-0.2 bndxy(3,1)+0.2 -1.2 1.2]);
axis equal; axis off
step_dom=gca;
save step_ns_plot.mat step_dom
close(2)
return
