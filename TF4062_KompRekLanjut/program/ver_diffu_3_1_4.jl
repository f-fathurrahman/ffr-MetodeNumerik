...
function main()
    ...
    u, x, t = diffusion_1d_implicit(L, Nx, T, Nt, α, initial_cond, bx0, bxL, source_term)
    #u, x, t = diffusion_1d_CN(L, Nx, T, Nt, α, initial_cond, bx0, bxL, source_term)

    #for n = 1:Nt
    #    plt.clf()
    #    plt.plot(x, u[:,n], marker="o")
    #    plt.ylim(0.0, 0.13)
    #    filename = @sprintf("IMG_ver_expl_%04d.png", n)
    #    println(filename)
