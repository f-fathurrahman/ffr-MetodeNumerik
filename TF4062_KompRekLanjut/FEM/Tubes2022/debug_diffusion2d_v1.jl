...
# number of time steps to perform (vary this, set to Ntime=1 for debugging)
Ntime = 1
...
for itime in 1:Ntime
    t = t + dt # update time
    b[:] = .... # form rhs global vector
    #
    LHS[bcdof,:] .= .... # zero the boundary equations
    for i in bcdof
        LHS[i,i] = ....
    end
    b[bcdof] = .... # set boundary values
    #
    # Solve linear system
    Tnum[:] = LHS \ b 
end
...
# Catatan (opsional untuk dikerjakan):
# Loop di atas dapat dioptimasi sehingga waktu kalkulasi yang diperlukan lebih kecil
# dengan menggunakan dekomposisi LU pada matriks LHS sebelum loop terhadap waktu.
# Matriks LHS tidak berubah selama evolusi sehingga bagian aplikasi syarat batas dapat dikeluarkan
# dari loop terhadap waktu. Lihat fungsi lu dan ldiv! pada pustaka LinearAlgebra
...
println("MAE = ", diffT) # sekitar 0.xxxx, bergantung paramter numerik yang digunakan
