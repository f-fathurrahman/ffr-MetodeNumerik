function [a,b,c]=start_search_x2D(a,b,c)
% function [a,b,c,search_depth,num_perm]=start_search_x(a,b,c)
% HG Tue Dec 21 10:32:57 JST 1999
% PURPOSE: driver to sort single array using a 
%  vectorizable sorting algorithm calls inc_sort_x
...
%  b: companion array (index)
%  c: companion array (upper or lower bounding box +-10
% ALGORITHM: 
...
%    sum_a=sum(a(2:end-1));
    ...
    inc_sort_x2D(wrong_position_list_2,wrong_position_index_2,a,b,c);
...
%    sum_a=sum(a(2:end-1));
    ...
    inc_sort_x2D(wrong_position_list_1,wrong_position_index_1,a,b,c);
%    if ~(abs(sum_a-sum(a(2:end-1)))*1e8<abs(sum_a))
%       sum(a(2:end-1))
%      error('Control sum of a_x-entries wrong 2')
%    end
%    search_depth=search_depth+1;
    if wrong_position_index_1==0 % verified ! 0, not 1
      break
