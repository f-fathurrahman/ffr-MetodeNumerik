...
function [handles] = read_data_file()
    ...
    handles.nc_x = fdtd_domain.node_coordinates_xe;
    handles.nc_y = fdtd_domain.node_coordinates_ye;
    handles.nc_z = fdtd_domain.node_coordinates_ze;
    handles.nc_xh = fdtd_domain.node_coordinates_xh;
    handles.nc_yh = fdtd_domain.node_coordinates_yh;
    handles.nc_zh = fdtd_domain.node_coordinates_zh;
end
...
function [handles] = initialize_plotting_arrays(handles)
    ...
    [x1, y1] = meshgrid(nc_x(1:nx),nc_y);
    [x4, y4] = meshgrid(nc_x(2:nx+1),nc_y);
    x1 = reshape(x1,1,[]).';
    y1 = reshape(y1,1,[]).';
    x4 = reshape(x4,1,[]).';
    y4 = reshape(y4,1,[]).';    
    
    x = [x1 x1+dxt x4-dxt x4 x4-dxt x1+dxt];
    y = [y1 y1-dyt y1-dyt y1 y1+dyt y1+dyt];
    x = reshape(x.',1,[]).';
    y = reshape(y.',1,[]).';
    handles.vertices_xy_x_x = x;    
    handles.vertices_xy_x_y = y;
    ...
    [x1, y1] = meshgrid(nc_x,nc_y(1:ny));
    [x4, y4] = meshgrid(nc_x,nc_y(2:ny+1));
    x1 = reshape(x1.',1,[]).';
    y1 = reshape(y1.',1,[]).';
    x4 = reshape(x4.',1,[]).';
    y4 = reshape(y4.',1,[]).';    
    
    x = [x1 x1-dxt x1-dxt x1 x1+dxt x1+dxt];
    y = [y1 y1+dyt y4-dyt y4 y4-dyt y1+dyt];
    x = reshape(x.',1,[]).';
    y = reshape(y.',1,[]).';
    handles.vertices_xy_y_x = x;
    handles.vertices_xy_y_y = y; 
    ...
    [y1, z1] = meshgrid(nc_y(1:ny),nc_z);
    [y4, z4] = meshgrid(nc_y(2:ny+1),nc_z);
    y1 = reshape(y1,1,[]).';
    z1 = reshape(z1,1,[]).';
    y4 = reshape(y4,1,[]).';
    z4 = reshape(z4,1,[]).';    
    ...
    y = [y1 y1+dyt y4-dyt y4 y4-dyt y1+dyt];
    z = [z1 z1-dzt z1-dzt z1 z1+dzt z1+dzt];
    y = reshape(y.',1,[]).';
    z = reshape(z.',1,[]).';
    handles.vertices_yz_y_y = y;    
    handles.vertices_yz_y_z = z;
    ...
    [y1, z1] = meshgrid(nc_y,nc_z(1:nz));
    [y4, z4] = meshgrid(nc_y,nc_z(2:nz+1));
    y1 = reshape(y1.',1,[]).';
    z1 = reshape(z1.',1,[]).';
    y4 = reshape(y4.',1,[]).';
    z4 = reshape(z4.',1,[]).';    
    ...
    y = [y1 y1-dyt y1-dyt y1 y1+dyt y1+dyt];
    z = [z1 z1+dzt z4-dzt z4 z4-dzt z1+dzt];    
    y = reshape(y.',1,[]).';   
    z = reshape(z.',1,[]).';
    handles.vertices_yz_z_y = y;
    handles.vertices_yz_z_z = z; 
    ...
    [z1, x1] = meshgrid(nc_z(1:nz),nc_x);
    [z4, x4] = meshgrid(nc_z(2:nz+1),nc_x);
    z1 = reshape(z1,1,[]).';
    x1 = reshape(x1,1,[]).';
    z4 = reshape(z4,1,[]).';
    x4 = reshape(x4,1,[]).';    
    ...
    x = reshape(x.',1,[]).';
    z = reshape(z.',1,[]).';
    handles.vertices_zx_z_x = x;
    handles.vertices_zx_z_z = z;
    ...
    [z1, x1] = meshgrid(nc_z,nc_x(1:nx));
    [z4, x4] = meshgrid(nc_z,nc_x(2:nx+1));
    z1 = reshape(z1.',1,[]).';
    x1 = reshape(x1.',1,[]).';
    z4 = reshape(z4.',1,[]).';
    x4 = reshape(x4.',1,[]).';    
    ...
    z = reshape(z.',1,[]).';
    x = reshape(x.',1,[]).';
    handles.vertices_zx_x_z = z;
    handles.vertices_zx_x_x = x; 
    ...
    [x1, y1] = meshgrid(nc_x(1:nx+1),nc_y(2:ny+1));
    [x4, y4] = meshgrid(nc_x(2:nx+2),nc_y(2:ny+1));
    x1 = reshape(x1,1,[]).';
    y1 = reshape(y1,1,[]).';
    x4 = reshape(x4,1,[]).';
    y4 = reshape(y4,1,[]).';    
    ...
    x = reshape(x.',1,[]).';
    y = reshape(y.',1,[]).';    
    handles.mu_vertices_xy_x_x = x;
    handles.mu_vertices_xy_x_y = y;
    ...
    [x1, y1] = meshgrid(nc_x(2:nx+1), nc_y(1:ny+1));
    [x4, y4] = meshgrid(nc_x(2:nx+1), nc_y(2:ny+2));
    x1 = reshape(x1.',1,[]).';
    y1 = reshape(y1.',1,[]).';
    x4 = reshape(x4.',1,[]).';
    y4 = reshape(y4.',1,[]).';    
    ...
    x = reshape(x.',1,[]).';
    y = reshape(y.',1,[]).';    
    handles.mu_vertices_xy_y_x = x;
    handles.mu_vertices_xy_y_y = y;
    ...
    [y1, z1] = meshgrid(nc_y(1:ny+1),nc_z(2:nz+1));
    [y4, z4] = meshgrid(nc_y(2:ny+2),nc_z(2:nz+1));
    y1 = reshape(y1,1,[]).';
    z1 = reshape(z1,1,[]).';
    y4 = reshape(y4,1,[]).';
    z4 = reshape(z4,1,[]).';    
    ...
    y = reshape(y.',1,[]).';
    z = reshape(z.',1,[]).';    
    handles.mu_vertices_yz_y_y = y;
    handles.mu_vertices_yz_y_z = z;
    ...
    [y1, z1] = meshgrid(nc_y(2:ny+1), nc_z(1:nz+1));
    [y4, z4] = meshgrid(nc_y(2:ny+1), nc_z(2:nz+2));
    y1 = reshape(y1.',1,[]).';
    z1 = reshape(z1.',1,[]).';
    y4 = reshape(y4.',1,[]).';
    z4 = reshape(z4.',1,[]).';    
    ...
    y = reshape(y.',1,[]).';
    z = reshape(z.',1,[]).';  
    handles.mu_vertices_yz_z_z = z;
    handles.mu_vertices_yz_z_y = y;
    ...
    [z1, x1] = meshgrid(nc_z(1:nz+1),nc_x(2:nx+1));
    [z4, x4] = meshgrid(nc_z(2:nz+2),nc_x(2:nx+1));
    z1 = reshape(z1,1,[]).';
    x1 = reshape(x1,1,[]).';
    z4 = reshape(z4,1,[]).';
    x4 = reshape(x4,1,[]).';    
    ...
    handles.mu_vertices_zx_z_z = z;
    handles.mu_vertices_zx_z_x = x;
    ...
    [z1, x1] = meshgrid(nc_z(2:nz+1), nc_x(1:nx+1));
    [z4, x4] = meshgrid(nc_z(2:nz+1), nc_x(2:nx+2));
    z1 = reshape(z1.',1,[]).';
    x1 = reshape(x1.',1,[]).';
    z4 = reshape(z4.',1,[]).';
    x4 = reshape(x4.',1,[]).';    
    ...
    z = reshape(z.',1,[]).';
    x = reshape(x.',1,[]).';  
    handles.mu_vertices_zx_x_x = x;
    handles.mu_vertices_zx_x_z = z;
end
