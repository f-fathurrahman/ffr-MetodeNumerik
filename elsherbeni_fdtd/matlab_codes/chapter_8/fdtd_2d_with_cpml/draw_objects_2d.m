...
for i=1:number_of_sampled_magnetic_fields
    ...
    line([x x],[y y],'marker','+','MarkeredgeColor','r');
    text(x,y,[' H_' num2str(i)]);
end
...
if is_pml_xn
    ...
    v(:,2) = v(:,2) * (uy - ly) + ly;
    patch('Vertices',v,'Faces',faces,...
          'FaceColor',pml_RGB,'facealpha',0.3);
end
if is_pml_xp
...
if strcmp(boundary.type_xn, 'cpml')
    dlxc = dlx + dx * boundary.cpml_number_of_cells_xn;
    patch([dlx; dlxc; dlxc; dlx], [dly; dly; duy; duy], [0; 0; 0; 0], 'facecolor','g','facealpha',0.3)
end
if strcmp(boundary.type_xp, 'cpml')
    duxc = dux - dx * boundary.cpml_number_of_cells_xp;
    patch([dux; duxc; duxc; dux], [dly; dly; duy; duy], [0; 0; 0; 0], 'facecolor','g','facealpha',0.3)
end
if strcmp(boundary.type_yn, 'cpml')
    dlyc = dly + dy * boundary.cpml_number_of_cells_yn;
    patch([dlx; dlx; dux; dux], [dly; dlyc; dlyc; dly], [0; 0; 0; 0], 'facecolor','g','facealpha',0.3)
end
if strcmp(boundary.type_yp, 'cpml')
    duyc = duy - dy * boundary.cpml_number_of_cells_yp;
    patch([dlx; dlx; dux; dux], [duy; duyc; duyc; duy], [0; 0; 0; 0], 'facecolor','g','facealpha',0.3)
end
