...
def Y_l_J_u(l, ncut, theta_j):
    ...
    for n in range(al, ncut + 1):
        ...
        for m in range(al, ncut + 1):
            mf = scs.factorial((m - al), exact=True) / \
                    scs.factorial((m + al), exact=True)
            nf = scs.factorial((n - al), exact=True) / \
                    scs.factorial((n + al), exact=True)
            ...
            Y_l_J_m = Y_l_J_m + [c_n_m * sqa.quad(y_n_m, theta_j, np.pi, \
                                        epsabs=1.0e-06, epsrel=1.0e-06)[0]]
