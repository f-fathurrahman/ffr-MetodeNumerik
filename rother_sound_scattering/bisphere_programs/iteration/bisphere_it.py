#            Program "bisphere_it.py"
#
# This program calculates the plane wave scattering behaviour (zero, first 
# and second order iteration) on a bisphere arbitrarily oriented in the 
# laboratory frame. The configuration is fixed by the distance "b", and 
# the Eulerian angles "\alpha" and "\theta_p". This program is not based 
# on a translation of the laboratory frame into the body frame but only 
# on a rotation and a shift along the new z-axis after the rotation!
#
...
fobj = open("input_data_bisphere_it.txt", "r")
...
if (para0 == 's' and para1 == 's'):
    ...
    tm_0 = bas.tm_p(n1cut, beta_0, a0, k, k_p_k_0, rho_p_0)
    tm_1 = bas.tm_s(n1cut, beta_1)
...
# Iterative solution in each local system:
...
for l1 in range(-n1cut, n1cut + 1):
    y_l1 = sm_l_i_j_real[np.abs(l1)]
    y_dh = dh_n_l[l1 + n1cut]
    q_n1_0 = []
    for n1 in range(np.abs(l1), n1cut + 1):
        ...
        for n0 in range(0, n1cut + 1):
            ...
            q2 = q2 + (-1)**n0 * d_n0 * S_hin * D_hin
        q2 = q2 * q1
        q_n1_0 = q_n1_0 + [q2]
    q_l1_n1_0 = q_l1_n1_0 + [q_n1_0]

c_l0_m0_1 = bas.Q12(n1cut, alpha, theta_p, sm_l_i_j, t_o, q_l1_n1_0)    
q_l1_n1_1 = bas.C21(n1cut, alpha, theta_p, sm_l_i_j, t_o1, c_l0_m0_0)   

c_it_1 = []
for l in range(-n1cut, n1cut + 1):
    v1 = c_l0_m0_0[l + n1cut]
    v2 = c_l0_m0_1[l + n1cut]
    v_it_1 = np.array(v1) - np.array(v2)
    c_it_1 = c_it_1 + [v_it_1]
q_it_1 = []
for l in range(-n1cut, n1cut + 1):
    w1 = q_l1_n1_0[l + n1cut]
    w2 = q_l1_n1_1[l + n1cut]
    w_it_1 = np.array(w1) - np.array(w2)
    q_it_1 = q_it_1 + [w_it_1]
       
cc1 = bas.C21(n1cut, alpha, theta_p, sm_l_i_j, t_o1, c_it_1)
qq1 = bas.Q12(n1cut, alpha, theta_p, sm_l_i_j, t_o, q_it_1)

c_l0_m0_2 = bas.Q12(n1cut, alpha, theta_p, sm_l_i_j, t_o, cc1)    
q_l1_n1_2 = bas.C21(n1cut, alpha, theta_p, sm_l_i_j, t_o1, qq1)
                          
c_final = []
for l0 in range(-n1cut, n1cut + 1):
    v1 = c_l0_m0_0[l0 + n1cut]
    v2 = c_l0_m0_1[l0 + n1cut]
    v3 = c_l0_m0_2[l0 + n1cut]
    if LS == 0:
        v = v1
    elif LS == 1:
...
q_final = []
for l1 in range(-n1cut, n1cut + 1):
    w1 = q_l1_n1_0[l1 + n1cut]
    w2 = q_l1_n1_1[l1 + n1cut]
    w3 = q_l1_n1_2[l1 + n1cut]
    if LS == 0:
        w = w1
    elif LS == 1:
...
qw_l1s_nu1 = []
for l1s in range(-n1cut, n1cut + 1):
    qw_nu1 = []
    for nu1 in range(np.abs(l1s), n1cut + 1):
        zw0 = 0.0
        for l1 in range(-n1cut, n1cut + 1):
            y_l1 = sm_l_i_j_real[np.abs(l1)]
            y_l1_nu1 = y_l1[nu1]
            D_rueck = bas.drm(nu1, l1s, l1, alpha, theta_p, 0.)
            q_l1 = q_final[l1 + n1cut]
            zw = 0.0            
            for n1 in range(np.abs(l1), n1cut + 1):
                n1z = n1 - np.abs(l1)
                S_rueck = y_l1_nu1[n1]
                zw = zw + q_l1[n1z] * S_rueck * D_rueck
            zw0 = zw0 + zw
        qw_nu1 = qw_nu1 + [zw0]
    qw_l1s_nu1 = qw_l1s_nu1 + [qw_nu1]
...
for i in range(0, theta_l1):
    psi_s = 0.0    
    for l0 in range(-n1cut, n1cut + 1):
        c_l0 = c_final[l0 + n1cut]
        q_l1s = qw_l1s_nu1[l0 + n1cut]
        for m0 in range(np.abs(l0), n1cut + 1):
            m0z = m0 - np.abs(l0)
            pref = (-zi)**m0
            if i <= 180:
            ...
            psi_s = psi_s + (c_l0[m0z] + (-1)**m0 * q_l1s[m0z]) * \
                             pref * Y_l0_m0
    psi_s = pf * psi_s
...
if plot_para == 'lg':
    plt.yscale('log')
    if pl_pm1 == 'y':
        plt.plot(theta, dscross_zw, color = 'r', linewidth=2.0, label = \
             "2 * (Mie_a0 + Mie_a1)")
