function [Ephi, Etheta] = mie_sphere_diel(freq, a, epsr, phio, thetao, r)
% ***********************************************************************
...
dbesselkda = (besselj(n-1/2,kda)-besselj(n+3/2,kda))/2;
besselkda = besselj(n+1/2,kda);
dsbesselkda = (pi/4)*((pi*kda/2)^(-1/2))*besselkda + sqrt(pi*kda/2)*dbesselkda;
sbesselkda = sqrt(pi*kda/2)*besselkda;

an = 1j.^(-n).*(2*n+1)./(n.*(n+1));

bn = an .* (-edm0*dsbesselka.*sbesselkda + e0md*sbesselka.*dsbesselkda) ./ ...
          (edm0*dshankelka.*sbesselkda - e0md*shankelka.*dsbesselkda);

cn = an .* (-edm0*sbesselka.*dsbesselkda + e0md*dsbesselka.*sbesselkda) ./ ...
          (edm0*shankelka.*dsbesselkda - e0md*dshankelka.*sbesselkda);
