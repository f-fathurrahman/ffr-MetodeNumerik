function [qual] = tetra_quality1(x,y,z)
% It computes the quality measure Q1 for tetrahedral elements.
% Input:
...
v24 = [x(:,4)-x(:,2) y(:,4)-y(:,2) z(:,4)-z(:,2)];

v1213 = cross(v12,v13,2);
v1214 = cross(v12,v14,2);
v1314 = cross(v13,v14,2);
v2324 = cross(v23,v24,2);

v1213norm = sqrt(v1213(:,1).^2 + v1213(:,2).^2 + v1213(:,3).^2);
v1214norm = sqrt(v1214(:,1).^2 + v1214(:,2).^2 + v1214(:,3).^2);
v1314norm = sqrt(v1314(:,1).^2 + v1314(:,2).^2 + v1314(:,3).^2);
v2324norm = sqrt(v2324(:,1).^2 + v2324(:,2).^2 + v2324(:,3).^2);

% radius of inscribed sphere
ris = (6*vol) ./ (v1213norm + v1214norm + v1314norm + v2324norm);
