function [conn,elm2edge_conn,edge2node_conn,x,y,z,exmid,eymid,ezmid,...
    ...
    = meshgen_circprism(delh,scat_type,rs,hheightz,fsdim,pmldim,huygdim)
...
hheightz = round(hheightz/delh)*delh; 
...
% *************************************************************************
x = []; y = []; z = [];
scatin_x = []; scatin_y = []; scatin_z = [];

cx = 0; cy = 0;
for r = delh:delh:rs-delh
    alfac = 2*asin(delh*0.5/r);
    N = floor(pi/alfac);
 ...
 if strcmpi(scat_type, 'diel') % dielectric    
    scatin_x = cx; scatin_y = cy; scatin_z = zeros(size(cx));
    for h = delh:delh:hheightz-delh
        scatin_x = cat(2, scatin_x, cx, cx);
        scatin_y = cat(2, scatin_y, cy, cy);  
        scatin_z = cat(2, scatin_z, h*ones(size(cx)), -h*ones(size(cx)));
    end  
...
cx2 = cx; cy2 = cy; hz = hheightz;
for r = rs:delh:rpmlout
    alfac = 2*asin(delh*0.5/r);
    N = floor(pi/alfac);
    alfa = cat(2, (pi/N)*(0:N), -fliplr((pi/N)*(1:N-1)));
    ...
    x1 = xt; y1 = yt; z1 = zeros(size(xt));
    for h = delh:delh:hz-delh
        
        x1 = cat(2, x1, xt, xt);
        y1 = cat(2, y1, yt, yt);
        z1 = cat(2, z1, h*ones(size(xt)), -h*ones(size(xt)));
        
    end
    
    x1 = cat(2, x1, cx2, cx2);
    y1 = cat(2, y1, cy2, cy2);
    z1 = cat(2, z1, hz*ones(size(cx2)), -hz*ones(size(cx2)));
...
if strcmpi(scat_type, 'pec')    % PEC
    ...
    ymid = mean(y(conn),2);  
    zmid = mean(z(conn),2);    
    discard_elm = find((sqrt(xmid.^2 + ymid.^2) < rs-1e-6) & (zmid < hheightz-1e-6) & (zmid > -hheightz+1e-6));    
...
% *************************************************************************
scatin_elm = [];  scatin_edge = [];
if strcmpi(scat_type, 'diel') % diel   
    scatin_elm = find((sqrt(xmid.^2 + ymid.^2) < rs) & (zmid < hheightz) & (zmid > -hheightz));
    scatin_edge = find((sqrt(exmid.^2 + eymid.^2) < rs+1e-6) & (ezmid < hheightz+1e-6) & (ezmid > -hheightz-1e-6));
end
...
% *************************************************************************
temp_node = find((sqrt(x.^2 + y.^2) < rpmlin+1e-6) & (z < max(pmlbin_z)+1e-6) & (z > min(pmlbin_z)-1e-6));
nodes0 = ones(1,N); 
nodes0(temp_node) = 0;
...
for elm = 1:M
    ...
    if strcmpi(scat_type, 'pec') % PEC
       ...
       if (count >= 3) && ~((sqrt(xmid(elm)^2 + ymid(elm)^2) < rs) && (zmid(elm) < hheightz) && (zmid(elm) > -hheightz))
...
%               keyboard
       ...
       if (count >= 3) && ~((sqrt(xmid(elm)^2 + ymid(elm)^2) < rhuyg) && (zmid(elm) < max(huygb_z)) && (zmid(elm) > min(huygb_z)))
...
%     xlabel('x (m)');  ylabel('y (m)'); zlabel('y (m)');
    ...
    plot3(exmid(scatin_edge),eymid(scatin_edge),ezmid(scatin_edge),'c.')    
