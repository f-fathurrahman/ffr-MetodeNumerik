...
%
% It solves 1-D Poisson's equation with "linear" elements and 
...
xm = linspace(le/2, L-le/2, M)'; % coordinates of the midpoints of 
                                 % the elements (size: Mx1)
...
Ae = [1 -1; -1 1]*eps/le; % element matrix
...
for e = 1:M  % loop over all elements
    be = [1;1]*0.5*le*rhov(e); % element b-vector
...
% Error comparing potentials found by FEM and the analytical method
%**********************************************************************
err = 100*norm(V-Ve).^2/norm(Ve).^2;
disp(['Error (%) = ' sprintf('%.4e', err)]);

% Error comparing NODAL potentials found by FEM and the analytical method
%*************************************************************************
...
plot(x2,V,'k', 'Linewidth', 2) % plot the potential
hold on
plot(x2,Ve,'k--', 'Linewidth', 2) % plot the exact potential
grid on
xlabel('x (m)')
ylabel('Potential (V)')
...
figure, clf, whitebg('white'), set(gcf, 'Color', [1 1 1]);
stairs(x,[Ex; Ex(end)],'k', 'Linewidth', 2) % plot the electric field
hold on
plot(xm2,Exe,'k--', 'Linewidth', 2) % plot the exact electric field
grid on
xlabel('x (m)')
