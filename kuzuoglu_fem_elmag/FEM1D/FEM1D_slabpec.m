%% Name: FEM1D_slabpec.m 
% Scattering from a PEC-Backed Dielectric Slab (Sec. 4.6.2)
%
...
% Analytical value of the magnitude of reflection coefficient is also 
% displayed and compared with the numerical solution.
%
%
...
er = 2.7;     % dielectric constant of the slab
...
xa = L-d;  % x-coordinate of the slab (left)
xb = L;    % x-coordinate of the slab (right)
...
x = [x1; x2(2:end)];       % array of coordinates (size: Nx1)
...
%************************************************
A(1,1) = A(1,1) + 1j*k0*p(1);

% Imposition of the Dirichlet BC boundary condition at x=L
%*********************************************************
A(N,:) = 0;   % make the last row of A matrix zero
A(N,N) = 1;   % make the diagonal term of the A matrix 1
b(N) = -exp(-1j*k0*x(end));  % assign the last row of b to -uinc  
...
% Compute the magnitude of reflection coefficient
R = abs(u(1));      % reflection coefficient
disp(['|R| = ' sprintf('%.4f', R)]);

% Compute the exact reflection coefficient
nu = nu0/sqrt(erc/mur); % ohm, intrinsic impedance of the slab
k = k0*sqrt(mur*erc);   % 1/meter, wavenumber in the slab

Re = -(nu0-1j*nu*tan(k*d)) / (nu0+1j*nu*tan(k*d));
Re = abs(Re);  % exact reflection coefficient
...
hold on
plot(x,abs(utot),'r', 'Linewidth', 2) % plot the total field
grid on
xlabel('x (m)')
ylabel('|Ez| (V/m)')
plot([L L], [0 2], 'k', 'Linewidth', 3)
legend('Slab region', 'Scattered field', 'Total field', 'PEC wall')
