...
%
% It solves 1-D generalized Laplace’s equation with "quadratic" elements 
...
M = 4;         % number of quadratic elements
...
conn = [(1:2:N-2)' (2:2:N-1)' (3:2:N)']; % connectivity matrix (size:Mx3)

% Material and source parameters
%*******************************
eps = e0*(1+xm/L);      % permittivity in each element (size: Mx1)
rhov = zeros(size(xm)); % zero array of charge density
 

%% ***********************************************************************
% MAIN BODY
%*************************************************************************
Ne = 3; % number of nodes in each element
...
for e = 1:M  % loop over all elements
    be = [0;0;0]; % zero element b-vector    
    %be = [1;4;1]*le*rhov(e)/6; % element b-vector (enable if rhov is nonzero)
    Ae = [7 -8 1; -8 16 -8; 1 -8 7]*eps(e)/(3*le); % element matrix    
...
i = 1; j = 1;
for e = 1:M % loop over all elements
    V(i:i+num-1) = u(j)*0.5*ksi.*(ksi-1) + u(j+1)*(1+ksi).*(1-ksi) + u(j+2)*0.5*ksi.*(ksi+1);
    i = i+num-1; 
    j = j+2;    
end
...
i = 1;  j = 1;
for e = 1:M % loop over all elements
    Ex(i:i+num-2) = (-2/le)*(u(j)*(ksim-0.5) + u(j+1)*(-2*ksim) + u(j+2)*(ksim+0.5));    
    i = i+num-1; 
    j = j+2;
end
