...
% Mesh generation function for "Scattering from a PEC Circular Object
% coated by a Number of Dielectric Layers" using triangular elements
% Used in FEM2D_scat_layer.m
%
% INPUT:
...
% layerb_node : Array containing the nodes on the surfaces of the layer boundaries
% layer : Structure containing the layer elements for each layer
%
...
rlayer = obj.rlayer; % radius of dielectric layers (from inner to outer)
...
for r = rc:delh:rpmlout
    ...
    for ir = 1:Nlayer
        if (r < rlayer(ir)+1e-8) && (r > rlayer(ir)-1e-8)
            layer(ir).bx = x0;
            layer(ir).by = y0;
        end
    end
...
% *************************************************************************
% Find layer elements and nodes
% *************************************************************************
INPO = inpolygon(x,y, layer(Nlayer).bx, layer(Nlayer).by);
layerb_node = find(INPO == 1);

for ir = 1:Nlayer
    INPO = inpolygon(xmid,ymid, layer(ir).bx, layer(ir).by);
    layer(ir).elm = find(INPO == 1);
    
    if ir > 1
       elms0 = zeros(1,M);
       elms0(layer(ir).elm) = 1;
       for i = 1:ir-1
           elms0(layer(i).elm) = 0;
       end
       layer(ir).elm = find(elms0 == 1);
    end
end
...
if plot_flag
    ...
    plot(x(scatb_node), y(scatb_node),'k.', 'LineWidth',1.5)    
    plot(x(layerb_node), y(layerb_node),'r.', 'LineWidth',1.5)
    ...
    plot(xmid(scatb_elm), ymid(scatb_elm), '.')
    
    for ir = 1:Nlayer
        plot(xmid(layer(ir).elm), ymid(layer(ir).elm), 'c.')
    end
end
