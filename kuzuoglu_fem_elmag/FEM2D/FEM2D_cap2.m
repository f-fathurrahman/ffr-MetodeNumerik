%% Name: FEM2D_cap2.m 
% 2D Parallel-Plate Capacitor Problem with Dielectric Inhomogeneity (Sec. 5.5.2)
%
...
% Mesh Generation
%****************
Lx = .08; % m, length of the computational domain along x axis
Ly = .04; % m, length of the computational domain along y axis

dx = .02; % m, length of the dielectric block along x axis
dy = .02; % m, length of the dielectric block along y axis
...
if elm_type == 1  % triangular
   [conn,x,y,xmid,ymid,btop_node,bbottom_node,bleft_node,bright_node,d_elm] = meshgen_cap2(Lx,Ly,dx,dy,delx,dely);
elseif elm_type == 2  % quadrilateral
   [conn,x,y,xmid,ymid,btop_node,bbottom_node,bleft_node,bright_node,d_elm] = meshgenq_cap2(Lx,Ly,dx,dy,delx,dely);
else
...
er = 4;        % dielectric constant of the dielectric block

% Boundary conditions
%********************
Vtop = 1;     % Dirichlet BC on the top boundary
Vbottom = -1; % Dirichlet BC on the bottom boundary
...
BCvalues = [Vtop*ones(size(btop_node)); Vbottom*ones(size(bbottom_node))];  % function values at the boundary nodes
