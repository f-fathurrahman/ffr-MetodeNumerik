function [rcs, farf] = rcsTM(Ez, conn, x, y, bound_node, bound_elm, scat_type)
% It calculates RCS for TM mode
%
%
...
mur = 1; % relative permeability of the outer region (free-space)
...
for i = 1:length(bound_elm)
    ...
    Jz = (1/(1j*omg*mu0*mur))*normal_derivative(e, anx, any, x, y, conn, Ez);
    
    for phi = 0:359
        phir = phi*pi/180;
        Ef = exp(1j*k0*(xm*cos(phir)+ym*sin(phir)));
        
        rcs_term(phi+1) = rcs_term(phi+1) + Jz*delc*Ef;

        if strcmpi(scat_type, 'diel')
           Mxy = (anx*cos(phir)+any*sin(phir))*Ezm; 
           rcs_term2(phi+1) = rcs_term2(phi+1) + Mxy*delc*Ef;
        end
    end
    
end

farf1 = rcs_term*nu0*sqrt(k0/(8*pi*rfar))*exp(-1j*k0*rfar-1j*3*pi/4);
farf2 = rcs_term2*sqrt(k0/(8*pi*rfar))*exp(-1j*k0*rfar+1j*pi/4);
farf = farf1+farf2;
