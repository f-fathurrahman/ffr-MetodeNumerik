function [conn,z,x,pmlbinr_node,pmlboutr_node,pmlr_node, ...
    ...
    = meshgen_wg3(obj)
% Mesh generation function for "Line-source radiation in a parallel plate
% waveguide" using triangular elements 
% Used in FEM2D_wg3.m
%
% INPUT:
...
object_exist = obj.object_exist; % flag showing an object exists or not within the waveguide
cz = dz/2;                   % center of the object along z
cx = obj.cx;                 % center of the object along x
srcz = obj.srcz;             % line source location along z
srcx = obj.srcx;             % line source location along x
ls = obj.ls;                 % half-length of source region
...
ls = round(ls/delh)*delh; 
dx = round(dx/delh)*delh;   
dz = round(dz/delh)*delh;  
pmldim = round(pmldim/delh)*delh; 
cz = round(cz/delh)*delh; 
cx = round(cx/delh)*delh; 
srcz = round(srcz/delh)*delh;
srcx = round(srcx/delh)*delh;
...
INPO = inpolygon(zmid, xmid, [-ls ls ls -ls]+srcz, [-ls -ls ls ls]+srcx);
...
% source boundary
sourceb_node = find(((z < ls+srcz+1e-8) & (z > ls+srcz-1e-8) & ...
                     (x < ls+srcx+1e-8) & (x > -ls+srcx-1e-8)) | ...
                    ((z < -ls+srcz+1e-8) & (z > -ls+srcz-1e-8) & ...
                     (x < ls+srcx+1e-8) & (x > -ls+srcx-1e-8)) | ...
                    ((x < ls+srcx+1e-8) & (x > ls+srcx-1e-8) & ...
                     (z < ls+srcz+1e-8) & (z > -ls+srcz-1e-8)) | ...
                    ((x < -ls+srcx+1e-8) & (x > -ls+srcx-1e-8) & ...
...
% *************************************************************************
% Find boundary elements at the source boundary
% *************************************************************************
sourceb_elm = [];

nodesall = zeros(1, N);
nodesall(sourceb_node) = 1;
...
if plot_flag
    ...
    plot(z(pmlboutl_node), x(pmlboutl_node),'g.', 'LineWidth',1.5)
    plot(z(pmlbinl_node), x(pmlbinl_node),'m.', 'LineWidth',1.5)
    plot(z(pmll_node), x(pmll_node), 'y.', 'LineWidth',1.5)    
    ...
    plot(zmid(sourceb_elm), xmid(sourceb_elm), 'g.')     
    plot(z(sourceb_node), x(sourceb_node),'m.', 'LineWidth',1.5)
end
