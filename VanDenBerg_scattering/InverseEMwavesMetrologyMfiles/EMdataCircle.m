...
Z_sct   = sqrt(mu_sct/eps_sct);        a = input.a;  
...
% (3) Compute reflected fields at receivers (data) ------------------------
  E1data = zeros(input.NR,input.NS);  E2data = E1data;  H3data = E1data;
  for p = 1 : input.NR;    
    for q = 1:input.NS;  
      Er = 0;  Ephi = 0; ZH3 =0;
      for m = 1 : M;
  ...
  E1data(p,q) = E1;  E2data(p,q) = E2;   H3data(p,q) = ZH3;
    end % q_loop
  end % p_loop 