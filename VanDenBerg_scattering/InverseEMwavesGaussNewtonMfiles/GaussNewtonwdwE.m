...
% Compute Green function for sources and receivers and compute datadata     
...
% Start with estimated contrast sources
 ...
 for it = 1: itmaxGN 
  % Compute residual error, construct Jacobian matrix and update P
    P = P_it(it,:);  
    data_est    =  FunDataSctIntqwdwE(P,pG,input);
  ...
  % Compute numerical derivatives with respect to P   
    P(1) = P(1) + eps;  data_plus = FunDataSctIntqwdwE(P,pG,input);
      dCHI1_P{1} = (data_plus{1} - data_est{1}) /eps;  
      dCHI2_P{1} = (data_plus{2} - data_est{2}) /eps;  
    P(1) = P(1) - eps;
    P(2) = P(2) + eps;  data_plus = FunDataSctIntqwdwE(P,pG,input);
  ...
  % Compute residual error, construct Jacobian matrix and update P
    ...
    alpha  = 0.6180;              % use Golden ratio for damping  
    P = P + alpha * DeltaP ;         
    disp(['        P: ', num2str(P)]);
    P_it(it+1,:) = P; % save as global array for plotting
 end; % it_loop
 
 DisplayParameterswdwE;