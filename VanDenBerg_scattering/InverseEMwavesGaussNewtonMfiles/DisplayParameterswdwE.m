function DisplayParameterswdwE
global Pnom P_it 

set(figure(10), 'Position', [50 50 550 400] );  N = length(P_it(:,1))-1;
subplot(1,2,1)
  plot(0:N,Pnom(1)*ones(N+1,1),':k','Linewidth',2);    hold on;
  plot(0:N,P_it(:,1),'-r','Linewidth',2);       
...
subplot(1,2,2)
  ...
  plot(0:N,P_it(:,2),'-r','Linewidth',2);     
  title('P_2 = a','Fontsize',11); 
  xlabel('number of iterations','Fontsize',11); 
  axis('tight'); axis([ 0 N 30 70]);  grid on; 
  legend('P nominal ', 'P wdwE ','Location','NorthEast')