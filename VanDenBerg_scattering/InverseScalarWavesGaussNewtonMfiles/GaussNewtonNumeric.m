...
global Pnom  P_it 
input = init();   w = cell(1,input.NS); for q=1:input.NS; w{q} = 0; end 
itmax = 15; eps = 1e-6;
...
% (2) Start with estimated contrast sources
 ...
 for it = 1: itmax 
  % Compute residual error, construct Jacobian matrix and update P
    P = P_it(it,:);  
    data_est = FunDataSctIntEq(P,G_S,G_R,input);
  ...
  % Compute numerical derivatives with respect to P   
    P(1) = P(1) + eps;  data_plus = FunDataSctIntEq(P,G_S,G_R,input);
      dCHI_P{1} = (data_plus - data_est) /eps;       P(1) = P(1) - eps;
    P(2) = P(2) + eps;  data_plus = FunDataSctIntEq(P,G_S,G_R,input);
      dCHI_P{2} = (data_plus - data_est) /eps;       P(2) = P(2) - eps;
    P(3) = P(3) + eps;  data_plus = FunDataSctIntEq(P,G_S,G_R,input);
      dCHI_P{3} = (data_plus - data_est) /eps;       P(3) = P(3) - eps; 
    P(4) = P(4) + eps;  data_plus = FunDataSctIntEq(P,G_S,G_R,input);
