...
% (2) Compute reflected field at the receivers for all sources (data) -----
      xR(1,:) = input.xR(1,:) - input.xO(1);  % shifted origin 
      xR(2,:) = input.xR(2,:) - input.xO(2);  % shifted origin 
      xS(1,:) = input.xS(1,:) - input.xO(1);  % shifted origin 
      xS(2,:) = input.xS(2,:) - input.xO(2);  % shifted origin 
      ...
      dataCircle = 1/(2*pi) * data2D;                        clear data2D; 
      save DataAnalytic dataCircle; 